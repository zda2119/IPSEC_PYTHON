...
class CryptAlgo(object):
	...
	ZDA
	"""
	print("\nEncrypting an Encapsulating Security Payload: SPI - " + (str(esp.spi)))
...
class AuthAlgo(object):
	...
	print("Creating Digital Signature Using Library: " + str(mac))
	raw_input("Press Enter to Continue")
	    ...
	    #ZDA
	    mac_copy = mac.copy()
	    mac_copy.update(raw(pkt[ESP]))
	    print("Resulted Digital Signature: " + str(mac_copy.finalize()[:self.icv_size]) +"\n")
...
class SecurityAssociation(object):
	...
	print("Initializing the SecurityAssociation Class for: " + str(proto))
	print("Using Cryptographic Algorithm: " + str(crypt_algo) + "\n Key: " + str(crypt_key))
	print("Using Authentication Algorithm: " + str(auth_algo) + "\n Key: " + str(auth_key))
	raw_input("Press Enter to Continue\n")
	...
	print("Generating Blank ESP using _ESPPlain(): " + "\nSPI: " + str(esp.spi)
						+ "\n Sequence number: " + str(esp.seq)
						+ "\n IV: "+ str(esp.iv))
        ...
        esp.data = payload
	print("\nPayload & Data Before Encrpytion: " + str(payload))
	raw_input("Press Enter to Continue\n")
        esp.nh = nh

        esp = self.crypt_algo.pad(esp)
	print("Contents of ESP BEFORE Encryption for SPI " + str(esp.spi) + ":\n" + str(esp))
	raw_input("Press Enter to Continue")
        esp = self.crypt_algo.encrypt(self, esp, self.crypt_key,
                                      esn_en=esn_en or self.esn_en,
                                      esn=esn or self.esn)
	print("Contents of ESP AFTER Encryption for SPI " + str(esp.spi) + ":\n" + str(esp))
	raw_input("Press Enter To Continue")

	print("\nDigitally Signing Encrypted ESP for SPI " + str(esp.spi)+"\n")
	...
	print("\nCOMPUTED Final IP with Encrypted & Authenticated ESP and Authenticated AH: " + str(signed_pkt) +"\n")
        return signed_pkt
    ...
    def encrypt(self, pkt, seq_num=None, iv=None, esn_en=None, esn=None):
	    ...
	    print("ESP Encryption Identified - Passing information to crypt_algo._encrypt_esp() \n No Data Modified")
	    raw_input("Press Enter to Continue\n")
	    ...
	    print("AH Encryption Identified - Passing information to auth_algo._encrypt_ah()\n No Data Modified")
	    raw_input("Press Enter to Continue\n")
